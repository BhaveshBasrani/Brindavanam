...
import { X, Package, Clock, Truck, CheckCircle2, ShoppingBag, ShieldCheck, MapPin, CreditCard, ChevronRight } from 'lucide-react';
...
export const UserOrdersModal: React.FC<UserOrdersModalProps> = ({ isOpen, onClose, orders }) => {
  const { user } = useAuth();
      ...
      <div className="bg-white w-full max-w-3xl rounded-3xl shadow-2xl overflow-hidden relative border border-stone-200 animate-in fade-in duration-200 my-8">
        ...
        <div className="bg-[#3A5303] text-white p-6 relative flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          ...
          <div className="bg-white/10 backdrop-blur-md px-4 py-2 rounded-xl text-left border border-white/20 sm:self-center">
            ...
            <span className="text-xl font-serif text-[#94C000] font-bold">{orders.length} Orders (₹{totalSpent})</span>
          </div>
        </div>
          ...
          {orders.length === 0 ? (
            <div className="text-center py-16 space-y-4">
              ...
              <div className="space-y-1">
                <h3 className="text-lg font-serif text-stone-800">No Orders Placed Yet</h3>
                <p className="text-xs text-stone-500 max-w-sm mx-auto">
                  Your fresh wood-pressed oils and hand-churned A2 ghee orders will appear here once placed.
                </p>
              </div>
            </div>
          ) : (
            <div className="space-y-6">
              <h3 className="text-xs font-bold text-stone-900 uppercase tracking-wider font-serif border-b border-stone-200 pb-2">
                      ...
                      <div key={idx} className="flex justify-between items-center bg-white p-2.5 rounded-xl border border-stone-200/80">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-stone-500">{it.selectedVariant.weight} x {it.quantity}</p>
                          </div>
                        </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-[11px] text-stone-600 bg-white/60 p-3 rounded-xl border border-stone-200">
                    <div className="space-y-0.5">
                      <span className="font-bold text-stone-800 flex items-center">
                        <MapPin className="w-3 h-3 mr-1 text-[#3A5303]" /> Shipping Address:
                      </span>
                      <p className="font-medium text-stone-900">{ord.shippingAddress?.fullName}</p>
                      <p>{ord.shippingAddress?.addressLine1}, {ord.shippingAddress?.city}</p>
                    ...
                    <div className="space-y-0.5 sm:border-l border-stone-200 sm:pl-3">
                      ...
                      <p className="font-semibold text-stone-900">Method: {ord.paymentMethod}</p>
        ...
        <div className="bg-[#F7F6F2] p-4 border-t border-stone-200 flex justify-between items-center text-xs">
          <span className="text-stone-500 flex items-center">
            ...
            100% Certified Organic Farm Produce
          </span>
          <button
            onClick={onClose}
